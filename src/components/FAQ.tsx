    ...
    fetch(API, { cache: "no-store" })
      ...
      .then((json) => {
        ...
        if (Array.isArray(acf.faq_items) && acf.faq_items.length > 0) {
          setFaqs(acf.faq_items.map((f: any) => ({ q: f.question || "", a: f.answer || "" })));
        }
      })
      ...
      <div className="fn-container">
        ...
        <div className="text-center mb-10">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-4"
            style={{ background: "rgba(0,48,240,0.08)" }}>
            {faqImage
              ? <img src={faqImage} alt="FAQ" className="w-8 h-8 object-contain" />
              : <svg className="w-7 h-7" style={{ color: "#0030F0" }} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
          ...
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-gray-900 leading-tight mb-3">
            {titleFirst}<br />
            <span style={{ color: "#0030F0" }}>{titleSecond}</span>
          </h2>
            ...
            <div key={i} className="bg-white rounded-xl overflow-hidden"
              ...
              {open === i && (
                <div className="px-5 pb-5 border-t border-gray-100">
                  <div className="pt-4 text-gray-500 text-sm leading-relaxed">{faq.a}</div>
                </div>
              )}
            </div>
          ))}
        ...
        className="max-w-md sm:max-w-lg md:max-w-xl mx-auto rounded-2xl p-6 sm:p-8 text-center shadow-lg bg-white/80 backdrop-blur-md"
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <a href={btn1Link}
              className="px-6 py-2.5 rounded-lg text-sm font-semibold bg-white text-[#0030F0] border-2 border-white hover:bg-blue-50 transition-all">
              {btn1Text}
            </a>
            <a href={btn2Link}
              className="px-6 py-2.5 rounded-lg text-sm font-semibold border-2 text-white hover:bg-white hover:text-[#0030F0] transition-all"
