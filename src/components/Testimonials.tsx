...
interface Testimonial {
  stars: string[]; // array of rating image URLs
  quoteImage: string;
  quote: string;
...
interface Counter { number: string; title: string; description: string; }

const FB: Testimonial[] = [
  { stars: [], quoteImage: "", quote: "ACCESS PR has transformed our media outreach. We've seen a 300% increase in press coverage and our stories are reaching the right journalists faster than ever.", keyResultText: "300% increase in coverage", keyResultSub: "", author: "SC", authorName: "Sarah Chen", authorPosition: "VP of Communications", companyName: "TechVision Inc." },
  { stars: [], quoteImage: "", quote: "The analytics and reporting capabilities are game-changing. We can now measure our PR ROI accurately and prove the value of our communications efforts to leadership.", keyResultText: "250% improvement in ROI tracking", keyResultSub: "", author: "MR", authorName: "Michael Rodriguez", authorPosition: "Head of PR", companyName: "Global Dynamics" },
  { stars: [], quoteImage: "", quote: "ACCESS IR has streamlined our investor communications completely. The compliance features and automated distribution save us hours every week.", keyResultText: "15 hours saved weekly", keyResultSub: "", author: "JP", authorName: "Jennifer Park", authorPosition: "Investor Relations Director", companyName: "Innovation Corp" },
];
...
const StarSVG = () => (
  <svg className="w-4 h-4" style={{ color: "#0030F0" }} fill="currentColor" viewBox="0 0 20 20">
...
export default function Testimonials() {
  ...
  const [testimonials, setTestimonials] = useState<Testimonial[]>(FB);
  const [counters, setCounters] = useState<Counter[]>(FC);
          ...
          setTestimonials(acf.testimonials.map((t: any) => ({
            stars: Array.isArray(t.stars_rating)
              ? t.stars_rating.map((s: any) =>
                  typeof s.rating_image === "string" ? s.rating_image : (s.rating_image?.url || ""))
              : [],
    ...
    <section className="relative py-20 overflow-hidden" style={{ background: "#0030F0" }}>
      <div className="fn-container relative z-10">
        ...
        <div className="text-center mb-12">
          ...
          <div className="inline-block bg-white rounded-full px-8 py-2 mb-5">
            <span className="font-bold text-2xl md:text-3xl text-[#0030F0]">{highlightTitle}</span>
          </div>
          <p className="text-blue-200 text-sm md:text-base max-w-lg mx-auto leading-relaxed">{subtitle}</p>
        </div>

        {/* Cards */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          {testimonials.map((t, idx) => (
            <div key={idx} className="bg-white rounded-2xl p-6 shadow-md flex flex-col gap-4 hover:shadow-xl transition-all hover:-translate-y-1">
              <div className="flex items-center justify-between">
                <div className="flex gap-1">
                  {t.stars.length > 0
                    ? t.stars.map((url, i) => url
                        ? <img key={i} src={url} alt="star" className="w-4 h-4 object-contain" />
                        : <StarSVG key={i} />)
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 pt-12 pb-5">
          {counters.map((c, i) => (
            <div key={i} className="text-center rounded-2xl p-4 sm:p-6 bg-white border border-gray-200 shadow-sm hover:shadow-md transition">
